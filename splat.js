...
function initSplatShader() {
    ...
    splatShader.positionSplatUniform = gl.getUniformLocation(splatShader, "uPositionSplat");
    ...
    splatShader.timeUniform = gl.getUniformLocation(splatShader, "uTime");
...
Splat.prototype.setParameters = function(elapsed) {
    ...
    this.position[0] += 0.01*Math.sin(this.time); // permet de déplacer le splat sur l'axe X
}
...
Splat.prototype.sendUniformVariables = function() {
    ...
    if(this.loaded) {
    	gl.uniform3fv(splatShader.positionSplatUniform,this.position);
    	gl.uniform3fv(splatShader.couleurUniform,this.couleur);
      //gl.uniform3fv(splatShader.timeUniform,this.time);
